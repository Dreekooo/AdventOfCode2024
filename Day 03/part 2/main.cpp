...
int solve(std::string memory) {
    ...
    int closestOff;
    closestOff = memory.find("don't()");
    while ((currentIndex = memory.find("mul(", currentIndex + 1)) != std::string::npos) {
        if(currentIndex > closestOff && closestOff != std::string::npos){
            currentIndex = memory.find("do()", currentIndex + 1);
            if(currentIndex == std::string::npos)
                break;
            closestOff = memory.find("don't()", currentIndex + 1);
            continue;
        }
